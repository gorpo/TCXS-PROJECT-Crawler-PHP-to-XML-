...
#variavies iniciais
dados = open('ps2.php', 'r', encoding="utf-8").read()
...
for imagem in key_imagem:
    imagem = str(imagem).split('ps2/')[1].split('" width')[0]
...
with open('ps2.xml','a') as arquivo:
    ...
    for i in  range(len(dicionario_jogos)):
        ...
        texto1 = f"""
            <!-- id:{i} | PlayStation PS2 | TCXS PROJECT 2020 | {titulo.upper()} -->
            <Table key="tcxs_{i}">
                <Pair key="icon"><String>/dev_hdd0/game/HENBSTORE/USRDIR/IMAGES/ps2/{imagem}</String></Pair>
        ...
        <View id="tcxs_{i}_link">
            <Attributes>
                <Table key="tcxs_download{i}">
                    ...
                    <Pair key="prod_pict_path"><String>/dev_hdd0/game/HENBSTORE/USRDIR/IMAGES/ps2/{imagem}</String></Pair>
                </Table>
            </Attributes>
