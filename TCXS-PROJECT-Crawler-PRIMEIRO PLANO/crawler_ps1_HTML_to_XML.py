...
#variavies iniciais
dados = open('ps1_str.php', 'r', encoding="utf-8").read()
...
for imagem in key_imagem:
    imagem = str(imagem).split('ps1/')[1].split('" width')[0]
...
with open('ps1.xml','a') as arquivo:
    ...
    for i in  range(len(dicionario_jogos)):
        ...
        texto1 = f"""
            <!-- id:{i} | PlayStation PS1 | TCXS PROJECT 2020 | {titulo.upper()} -->
            <Table key="tcxs_{i}">
                <Pair key="icon"><String>/dev_hdd0/game/HENBSTORE/USRDIR/IMAGES/ps1/{imagem}</String></Pair>
        ...
        <View id="tcxs_{i}_link">
            <Attributes>
                <Table key="tcxs_download{i}">
                    ...
                    <Pair key="prod_pict_path"><String>/dev_hdd0/game/HENBSTORE/USRDIR/IMAGES/ps1/{imagem}</String></Pair>
                </Table>
            </Attributes>
